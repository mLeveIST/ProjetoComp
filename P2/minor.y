...
%%
file : PROGRAM decls START          { func = "main";
                                      ret = tPUB + tINT + tFUNC;
                                      IDpush(); }
       main END                     { IDpop();
                                      evaluate(binNode(PROGRAM, $2, binNode(FUNCTION, binNode(END, TID(func), TINT(ret)), binNode(FARGS, nilNode(NIL), $5)))); }
     | MODULE decls END             { evaluate(uniNode(MODULE, $2)); }
     ;
...
decl : qualif const vardecl         { $$ = uniNode(VAR, $3);
                                      $$->info = $1+$2+$3->info;
                                      isCte($$); }
     | FUNCTION qualif ftype ID     { isFunc(func = $4, ret = $2+$3+tFUNC);
                                      IDpush(); }
       fvars                        { IDchange(ret, $4, $6, 1); }
       eqbody                       { $$ = binNode(FUNCTION, binNode(END, TID($4), TINT(ret)), binNode(FARGS, $6, $8));
                                      IDpop();
                                      isFwd(func, ret, $8); }
     | FUNCTION qualif ftype ID     { isFunc( func = $4, ret = $2+$3+tFUNC);
                                      IDpush(); }
...
fvar : NUMBER ID                    { $$ = binNode(NUMBER, TID($2), nilNode(NIL));
                                      $$->info = tINT; }
     | STRING ID                    { $$ = binNode(STRING, TID($2), nilNode(NIL));
                                      $$->info = tSTR; }
     | ARRAY ID vdim                { $$ = binNode(ARRAY, TID($2), binNode(INTS, TINT($3), nilNode(NIL)));
                                      $$->info = tVEC; }
     ;
...
vardecl : NUMBER ID eqint           { $$ = binNode(NUMBER, TID($2), $3);
                                      $$->info = tINT; }
        | STRING ID eqstr           { $$ = binNode(STRING, TID($2), $3);
                                      $$->info = tSTR; }
        | ARRAY ID vdim eqvec       { $$ = binNode(ARRAY, TID($2), binNode(INTS, TINT($3), $4));
                                      $$->info = tVEC;
                                      isDim($2, $3, $4); }
        ;
...
eqint :                             { $$ = nilNode(NIL); }
      | ASSOC INT                   { $$ = intNode(INT, $2); }
      | ASSOC '-' INT               { $$ = intNode(INT, -$3); }
      ;
...
chars : char                        { $$ = binNode(CHARS, nilNode(NIL), $1);
                                      $$->info = $1->info; }
      | chars char                  { $$ = binNode(CHARS, $1, $2);
                                      $$->info = tSTR; }
      ;
...
main : fvars ';' instrs             { $$ = binNode(START, $1, $3); }
     | instrs                       { $$ = binNode(START, nilNode(NIL), $1); }
     ;

body : fvars ';' instrs ret         { $$ = binNode(START, $1, binNode(';', $3, $4)); }
     | instrs ret                   { $$ = binNode(START, nilNode(NIL), binNode(';', $1, $2)); }
     ;

ret : RETURN                        { $$ = uniNode(RETURN, nilNode(NIL));
                                      isRet(0); }
...
elifs :                                         { $$ = nilNode(NIL); }
      | elifs ELIF expr THEN block              { $$ = binNode(ELIF, $1, binNode(IF, $3, $5)); }
      ;
...
lval : ID                           { $$ = TID($1);
                                      $$->info = isId($1, 0, 0); }
     | ID '[' expr ']'              { $$ = binNode('[', $3, TID($1));
                                      $$->info = isId($1, $3, &$$->SUB(1)->info); }
     ;
...
expr : chars                        { $$ = $1; }
     ...
     | '-' expr %prec UMINUS        { $$ = uniNode(UMINUS, $2);
                                      $$->info = isUniInt($$, "-"); }
     | lval ASSOC expr              { $$ = binNode(ASSOC, $3, $1);
                                      $$->info = isAssign($$); }
     | ID '(' exprs ')'             { Node *n;
                                      int t = IDfind($1, (void**)&n);
                                      $$ = binNode('(', TID($1), $3);
                                      $$->user = n;
                                      $$->info = $$->SUB(0)->info = t;
                                      isCall($1, n, $3); }
...
static void isFwd(char *name, int type, Node *n) {
  ...
  if (attrib != NIL && tTYPE(type) == tVEC)
    attrib = n->SUB(1)->attrib;
...
static int isAddr(Node *n) {
  ...
  if (n->SUB(0)->attrib == '[')
    return IDfind(n->SUB(0)->SUB(1)->value.s,0);
...
static int isCall(char *name, Node *fargs, Node *eargs) {
  if (fargs->attrib != NIL && eargs->attrib != NIL) {
    ...
    if (tSUB(eargs,0) == tINT && eargs->SUB(0)->attrib == CHARS && eargs->SUB(0)->SUB(1)->value.i == 0)
      return pos;
...
static void isPrint(Node *n) {
  if (tTYPE(n->info) == tVOID)
    yyerror("can not print void expression");
}

static void isCycle() {
  if (!cycle)
    yyerror("'repeat' or 'stop' outside 'for' block");
}

static void isStr(Node *n) {
  if (n->info != tSTR)
    yyerror("invalid string initializer");
}
...
static void isRet(Node *n) {
  if (!n) {
    ...
    sprintf(errstr, "invalid return type (%s) for '%s' (%s)", types[tTYPE(n->info)], func, types[tTYPE(ret)]);
    yyerror(errstr);
  }
}
